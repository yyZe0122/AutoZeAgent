...
正式单元文件为 `packaging/systemd/autozeagent.service`（服务名 `autozeagent`，`ExecStart` 为 `autozeagentd`）。systemd 只管理一个长期运行的 `autozeagentd`；`autozeagent` 是按需执行的本地 CLI。Task、Run 和 Job 都是 `core.db` 中的逻辑对象，不需要额外 systemd unit。
