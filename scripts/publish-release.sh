...
#   ./scripts/publish-release.sh v0.1.0 --snapshot-only
#
# Requires: git, make. Optional: goreleaser (local snapshot preflight).
# Does NOT upload artifacts itself — pushes tag v* so Actions + GoReleaser publish.
set -euo pipefail
...
Options:
  ...
  --commit-paths MODE   release = whitelist packaging/release files; all = everything tracked+untracked (needs --yes)
  --message TEXT        Commit message when committing
  --tag-message TEXT    Annotated tag message
  --skip-check          Skip make check
  --skip-snapshot       Skip local goreleaser snapshot
  --snapshot-only       make check + snapshot only (no commit/tag/push)
  --force-tag           Delete local+remote tag if present, then recreate
  --dry-run             Print steps only
  --yes                 Confirm dangerous modes (--commit-paths all, --force-tag)
  -h, --help            This help
EOF
}
...
# Block staging obvious secrets if we commit
...
# Dirty tree check (ignore untracked ignored files; fail on modified/untracked)
...
if [[ "$SKIP_SNAPSHOT" -eq 0 ]]; then
  ...
  if command -v goreleaser >/dev/null 2>&1; then
    GR=$(command -v goreleaser)
  elif [[ -x /home/yyze/go/bin/goreleaser ]]; then
    GR=/home/yyze/go/bin/goreleaser
  elif [[ -x "${HOME}/go/bin/goreleaser" ]]; then
    GR="${HOME}/go/bin/goreleaser"
  fi
  if [[ -n "$GR" && -x "$GR" ]]; then
    log "goreleaser snapshot preflight ($GR)"
    if [[ "$DRY_RUN" -eq 1 ]]; then
      log "would run: $GR release --snapshot --clean --parallelism 1"
    else
      "$GR" release --snapshot --clean --parallelism 1
      log "snapshot archives:"
      ls -1 dist/autozeagent_* 2>/dev/null | head -20 || true
    fi
  else
    log "goreleaser not found; skip snapshot"
  fi
fi

if [[ "$SNAPSHOT_ONLY" -eq 1 ]]; then
  log "snapshot-only done (no push/tag)"
  exit 0
fi
...
if git rev-parse -q --verify "refs/tags/${TAG}" >/dev/null 2>&1; then
  if [[ "$FORCE_TAG" -eq 1 ]]; then
    [[ "$YES" -eq 1 ]] || die "--force-tag requires --yes"
    log "delete local tag ${TAG}"
    run "git tag -d \"$TAG\""
  else
    die "local tag ${TAG} already exists (use --force-tag --yes to replace)"
  fi
fi
...
if git ls-remote --tags "$REMOTE" "refs/tags/${TAG}" 2>/dev/null | grep -q .; then
  if [[ "$FORCE_TAG" -eq 1 ]]; then
    ...
    die "remote tag ${TAG} already exists (use --force-tag --yes to replace)"
  fi
fi
...
log "push tag ${TAG} (triggers GitHub Actions Release)"
run "git push \"$REMOTE\" \"$TAG\""

VER_NUM=${TAG#v}
cat <<EOF

==> publish kicked off for ${TAG}

  Actions:  https://github.com/yyZe0122/AutoZeAgent/actions
  Release:  https://github.com/yyZe0122/AutoZeAgent/releases/tag/${TAG}

  Expected assets (Pre-release):
  ...
  Body: ${NOTES}
  Install pin: AUTOZEAGENT_VERSION=${TAG}

  Verify when green:
    gh release view ${TAG} --repo yyZe0122/AutoZeAgent
EOF
