...
- 稳定错误码（现行）：`invalid_request`、`not_found`、`conflict`、`unavailable`；
- **已移除（交互 Planner 时代）**：`planning_pending`、`plan_changed`、`plan_document_unavailable` — 无生产者，Gateway 不再映射。
- `Retryable` 布尔属性；
...
- Task 输入错误为 `invalid_request`；重复或状态冲突为 `conflict`；缺失聚合为 `not_found`。
- Chat 不可用（未配置 provider/agent）为 `unavailable`；Approval/Grant 冲突与输入错误映射为 `conflict` / `invalid_request`。
...
当前明确可重试的应用结果是临时 `unavailable`（及 Gateway 数据库健康检查 503）。冲突与无效输入不可重试，调用方必须先刷新状态或修正请求。

不在本 ADR 中引入统一退避时长、自动重试执行器或跨进程错误协议；出现真实需求后再扩展。
