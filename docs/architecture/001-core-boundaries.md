...
AutoZeAgent 的生产核心是一个 `autozeagentd` 进程、一个 `autozeagent` CLI（含 TUI）和一个 `core.db`。Core 直接组合 Kernel、Planner、Agent Runner、Policy、Approval、Grant、Tool Broker、Scheduler、Event、Audit、Skill Catalog、Gateway 以及对应的窄应用服务。
