...
4. **Three channels stay distinct**:
   - **JSONL logs** — runtime diagnosis (`ymz logs`)
...
7. **CLI**: `ymz logs --run` / `--session` / `--task` / `--component` / `--level` / `--tail`.
8. **Tests vs logs**:
...
- A single chat turn should be reconstructible with `ymz logs --run <run_id>` across gateway → tasksubmission → chatsession → agent → tool_broker → terminal chatsession line.
...
```bash
export YMZ_LOG_LEVEL=debug   # optional
# TUI or CLI: send a message; note run_id from response / TUI metrics
ymz logs --run <run_id> --tail 500
ymz logs --session <session_id> --tail 500
ymz logs --task <task_id> --component tool_broker
```
