...
- 状态：Accepted
- 日期：2026-07-21
- 更新：2026-08-12（MCP stdio + 远程 HTTP/SSE 见 ADR-040；LSP 仍暂缓）
...
外部工具优先 MCP（stdio + 远程 Streamable HTTP / legacy SSE，ADR-040）；代码诊断优先 LSP（暂缓）。只实现当前实际需要的窄适配器。不得为了协议接入恢复通用 Module Runtime、私有 RPC、Supervisor 或独立数据库。
