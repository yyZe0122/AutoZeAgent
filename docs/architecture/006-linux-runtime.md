...
系统模式使用 `/etc/yunmengze`、`/var/lib/yunmengze`、`/run/yunmengze` 和 `/var/log/yunmengze`。用户模式使用扁平主目录 **`~/.yunmengze`**（配置/数据/日志/runtime 同根；`YMZ_HOME` 可覆盖），与 Claude/OpenCode 一类 agent 一致，不拆 XDG。生产路径中不再存在 ModuleDir 或独立模块数据目录。
