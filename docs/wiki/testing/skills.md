...
- Chat（`chatsession`）从 Task Skill 快照注入**独立** system 消息（ADR-036）；只读快照、不重读 `SKILL.md`。Prefix 另含技能目录（id — 一句话；ADR-052 R5）；正文仍快照 / `skill_view`。会话记忆走 in-process MemoryManager（ADR-044），不是独立 Memory 进程；
