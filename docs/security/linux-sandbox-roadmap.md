...
- 状态：Roadmap
- 日期：2026-07-13
- 适用范围：Linux 本地服务器上的 `process.exec` 与 Git 子进程
...
生产服务优先由 systemd 管理。每次高风险工具执行可进入 transient scope，建议评估以下属性：
...
- 检测宿主机是否使用 cgroups v2 unified hierarchy。
- 无 systemd 环境必须明确降级并记录 Audit，不能静默声称资源限制已启用。
- Broker 超时仍是上层最后期限，cgroup/systemd 是额外防线。
- transient unit 名称绑定 Tool Call ID，便于清理和审计。
