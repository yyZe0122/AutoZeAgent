...
- Optional **`models`** role map (ADR-045): `models.subagent` / `models.compact` as selection refs; omit → top-level `model` (main). `/model` only changes main; role map changes need daemon restart. Unknown keys fail load.
