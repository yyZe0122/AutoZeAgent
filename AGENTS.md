...
**Publish (root only on this host):** batch-commit the dirty tree by feature (do **not** one-shot `--commit-paths all` for a multi-feature dump), push, write `docs/changelog/vX.Y.Z.md`, then  
