...
**Publish (root only on this host):** batch-commit the dirty tree by feature (do **not** one-shot `--commit-paths all` for a multi-feature dump), push, write `docs/history/changelog/vX.Y.Z.md`, then  
`sudo -i && cd /home/yyze/projects/AutoZeAgent && ./scripts/publish-release.sh vX.Y.Z --yes`  
(`--commit-paths all` only for a leftover changelog commit.) Full runbook: [`docs/release.md`](docs/release.md) — do not invent parallel release steps.
...
| `internal/tui` | Bubble Tea UI (primary UX); Gateway-only; slash dispatch `cmds.go` + `cmds_*.go`; Elm `update.go` + `update_*.go`; **bubbles** + **lipgloss** cards; optional **glamour** / **bubblezone**; no list/viewport engine swap |
| `internal/kernel` | Session/Task/Run state machines (`model.go`) + repository (`repository.go` + `repository_{session,task}.go`) |
| `internal/tools` | Tool Broker (`broker.go`) + builtins; FS split `fs.go` + `fs_{read,write,search}.go`; nested `internal/executor` unimportable |
