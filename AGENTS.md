...
- Optional **`models`** role map (ADR-045): `models.subagent` / `models.compact` as selection refs; omit → top-level `model` (main). `/model` only changes **global** main; role map changes need daemon restart. Unknown keys (including `vision`) fail load. Chat Prefix = short identity (`internal/version.Version` + roles + no vision) then a pin-after `<env>` (model / workspace / date). User `<ConfigDir>/AGENTS.md` is a separate overlay. Skill catalog (id + one-liner) is injected into Prefix; bodies still load via `skill_view`. Sub-agents inherit the same AGENTS.md overlay. TUI header and `/status` show the same version.
...
- Dual-track tasks (OpenCode-style): both modes → `chatsession`; **agent** = write grants, **plan** = read-only. No interactive Planner/approval path. TUI Tab cycles plan|agent|auto (Auto is session stance, not a third execution_mode).
